...
Transcripts live at `${CODEX_HOME:-$HOME/.codex}/sessions/YYYY/MM/DD/rollout-<timestamp>-<thread-id>.jsonl`, and `CODEX_THREAD_ID` or `CODEX_SESSION_ID` holds the id of the current one. Every line is one JSONL event.
