...
At the end of the run, before handing back, check the log told the truth. Use the exact ID from `CODEX_THREAD_ID` or `CODEX_SESSION_ID` to locate this run's rollout under `${CODEX_HOME:-$HOME/.codex}/sessions/`. Don't scan unrelated rollout files; that reads unrelated private chats. If no path resolves, use a tight digest of the run. Walk the log against what actually happened:
