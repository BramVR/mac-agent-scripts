---
name: why
description: "Use for design rationale, regressions, postmortems, data-backed thresholds, and why code works a particular way."
---
...
Codex does not expose a read-only switch for collaboration agents. Explicitly instruct every agent not to edit files, change repository state, or perform external writes.
...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, current app context, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, inspect the current Codex tool catalog, available skills, and installed apps. For MCP servers that expose resources, use the available resource listings.
...
Launch matching investigators in concurrent waves of up to 3 agents, the maximum available alongside the lead. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one agent to cover multiple MCPs.

Each investigator uses the investigator contract above. The read-only instruction is a posture, not a sandbox. Codex collaboration agents retain their available tools and connectors.
...
Spawn one synthesizer agent using the synthesizer contract above. The synthesizer's quality check spot-verifies citations, which can require MCP access.
...
Substitute `JSON.stringify(investigatorFindings)` for `{ALL_INVESTIGATOR_FINDINGS_JSON}`. Never concatenate raw findings into the prompt.
