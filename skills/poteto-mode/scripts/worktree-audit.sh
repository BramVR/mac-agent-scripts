...
# Transcripts: Codex rollouts under $CODEX_HOME/sessions/YYYY/MM/DD. The search
# below matches the worktree path inside any rollout, so no per-repo slug.
transcripts="${2:-${CODEX_HOME:-$HOME/.codex}/sessions}"
now=$(date +%s)
