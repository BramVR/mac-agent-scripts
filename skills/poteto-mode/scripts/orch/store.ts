...
const TAKEOVER_LOCK_DIR = ".orch.lock.takeover";
...
interface ResolvedFrontierPr extends FrontierPr {
...
function parseGateOptions(value: string): readonly string[] {
  const options = requiredLine(value, "options")
    .split(",")
    .map((option) => option.trim());
  if (options.some((option) => option.length === 0)) {
    throw new UserError("gate options must not contain empty values");
  }
...
async function acquireLock(
  ...
  const takeoverPath = join(store, TAKEOVER_LOCK_DIR);
  ...
  const takeOver = async (
    ...
    const acquireTakeoverLock = async (): Promise<void> => {
      try {
        ...
        try {
          holder =
            (await readFile(join(takeoverPath, "owner"), "utf8")).trim() ||
            "unknown";
        } catch {
          holder = "unknown";
        }
        const ownerlessStaleTakeover =
          holder === "unknown" && (force || holderIsDead(observedHolder));
        if (!options.force && !holderIsDead(holder) && !ownerlessStaleTakeover) {
          throw new UserError(
            `store lock takeover already in progress by pid ${holder}`
          );
        }
        await rm(takeoverPath, { recursive: true, force: true });
        await mkdir(takeoverPath);
      }
      await writeFile(join(takeoverPath, "owner"), `${pid}\n`);
    };
    await acquireTakeoverLock();
    try {
      let currentHolder: string;
      try {
        currentHolder = (await readFile(path, "utf8")).trim() || "unknown";
      } catch (error) {
        if (errorCode(error) === "ENOENT") {
          await create();
          return;
      ...
      if (currentHolder !== observedHolder) {
        throw new UserError(`store lock changed to pid ${currentHolder}`);
      }
...
function summarize(
  ...
  return {
    ...
    unitRevision: createHash("sha256")
      .update(JSON.stringify(unitRows))
      .digest("hex"),
    ledgerRevision: createHash("sha256")
      .update(JSON.stringify(ledgerRows))
      .digest("hex"),
...
function previousSummary(raw: string): StatusSummary | null {
  ...
  if (
    ...
    typeof value.unitRevision !== "string" ||
    typeof value.ledgerRevision !== "string" ||
  ...
  return {
    unitStates,
    ledgerVerdicts,
    unitRevision: value.unitRevision,
...
function changed(before: StatusSummary | null, after: StatusSummary): string {
  ...
  if (before.unitRevision !== after.unitRevision) {
    result.push("unit details changed");
  }
  if (before.ledgerRevision !== after.ledgerRevision) {
    result.push("ledger details changed");
  }
...
function githubPullRequest({
  ...
  try {
    ...
    throw new UserError(
      `gh pr view ${pr} failed: ${errorMessage(error)}`
    );
  }
  let value: unknown;
  try {
  ...
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new UserError(`gh pr view ${pr} returned an invalid object`);
  }
  ...
  const baseBranch = row.baseRefName;
  if (number !== pr) {
    throw new UserError(`gh pr view ${pr} returned PR ${String(number)}`);
  }
  if (state !== "OPEN" && state !== "MERGED" && state !== "CLOSED") {
    throw new UserError(
      `gh pr view ${pr} returned invalid state ${String(state)}`
    );
  }
  if (typeof branch !== "string" || branch.length === 0) {
    throw new UserError(`gh pr view ${pr} returned an invalid head branch`);
  }
  if (typeof baseBranch !== "string" || baseBranch.length === 0) {
    throw new UserError(`gh pr view ${pr} returned an invalid base branch`);
  }
  if (typeof sha !== "string" || !/^[0-9a-f]{40,64}$/i.test(sha)) {
    throw new UserError(`gh pr view ${pr} returned an invalid head SHA`);
  }
  return { pr, branches: branch, sha, state, baseBranch };
}

function githubOpenPullRequestForBranch({
  branch,
  repo,
}: {
  branch: string;
  ...
  try {
    ...
    throw new UserError(
      `gh pr list --head ${branch} failed: ${errorMessage(error)}`
    );
  }
  let value: unknown;
  try {
    ...
    throw new UserError(`gh pr list --head ${branch} returned invalid JSON`);
  }
  if (!isUnknownArray(value)) {
    throw new UserError(`gh pr list --head ${branch} returned an invalid list`);
  }
  if (value.length === 0) return null;
  const row = value[0];
  if (!isRecord(row) || !Number.isSafeInteger(row.number)) {
    throw new UserError(`gh pr list --head ${branch} returned an invalid PR`);
  }
...
function resolveFrontier(
  ...
  const resolved = prs.map((pr) => githubPullRequest({ pr, repo }));
  const first = resolved[0];
  const omittedParent = githubOpenPullRequestForBranch({
    branch: first.baseBranch,
    repo,
  });
  if (omittedParent !== null) {
    throw new UserError(
      `PR ${first.pr} is not the stack root; its base ${first.baseBranch} belongs to open PR ${omittedParent}`
    );
  }
  for (const row of resolved) {
    try {
      execFileSync("git", ["cat-file", "-e", `${row.sha}^{commit}`], {
        cwd: repo,
        stdio: "ignore",
      });
    } catch {
      throw new UserError(
        `PR ${row.pr} head ${row.sha} is not available locally; fetch the PR heads before setting the frontier`
      );
    }
  }
  for (let index = 1; index < resolved.length; index += 1) {
    ...
    if (current.baseBranch !== previous.branches) {
      throw new UserError(
        `PR ${current.pr} targets ${current.baseBranch}, expected ${previous.branches} from PR ${previous.pr}`
      );
    }
    try {
      execFileSync(
        "git",
        ["merge-base", "--is-ancestor", previous.sha, current.sha],
      ...
      if (status === 1) {
        throw new UserError(
          `PR ${current.pr} head does not contain PR ${previous.pr} head`
        );
      }
      throw new UserError(
        `could not verify ancestry from PR ${previous.pr} to PR ${current.pr}: ${errorMessage(error)}`
      );
    }
  }
  return resolved.map(({ baseBranch: _baseBranch, ...row }) => row);
}
  ...
  return {
    ...
    inbox: {
      ...
      drain: async () => {
        await beginWrite();
        const inbox = join(store, "inbox");
        const stranded = (await readdir(store, { withFileTypes: true }))
          .filter(
            (entry) =>
        ...
        const rows = [
          ...(await Promise.all(stranded.map((path) => readPointers(path)))).flat(),
          ...(await readPointers(inbox)),
        ];
        const drained = join(
          store,
        ...
        for (const path of [...stranded, drained])
    ...
    gates: {
      park: async (params) => {
        await beginWrite();
        const options = parseGateOptions(params.options);
        const defaultAnswer = requiredLine(params.defaultAnswer, "default");
        if (!options.includes(defaultAnswer)) {
          throw new UserError("default must be one of the gate options");
        }
        const gate: OpenGate = {
          kind: "open",
      ...
      resolve: async (params) => {
        ...
        const answer = requiredLine(params.answer, "answer");
        if (!parseGateOptions(old.options).includes(answer)) {
          throw new UserError("answer must be one of the gate options");
        }
    ...
    frontier: {
      set: async (params) => {
        ...
        if (params.prs === undefined) {
          throw new UserError("--prs is required for Codex frontier discovery");
        }
        const pin = params.prs.map((pr) => positiveInteger(pr, "PR"));
        if (new Set(pin).size !== pin.length) {
