...
`$poteto-mode` applies to the current turn. Invoke it again on a later turn; Cursor's sticky `mode` and `reminder` metadata have no Codex skill equivalent. For long-running work, arm the objective with Codex Goal mode.

## Non-negotiables

**Start every multi-step task with a plan whose first item is to read the Principles section below in full.** The principles ground every trigger here. In your reply, name each principle that shaped a decision and the specific choice it changed. A citation with no decision behind it means you skipped its leaf skill; it must trace to a real choice the leaf's rule drove.
...
- Contested design → the **interrogate** skill (independent-agent adversarial) before shipping.
...
- Before commit → the **autoreview** skill (`$autoreview`).
- Before review → the **no-comments** skill (`$no-comments`).
- Shipping UI / IDE / CLI → the matching verification skill or tool. Use a project `verify-<app>` skill when one exists. Browser, Electron, and web UIs use `$browser-use` or the in-app browser. Native macOS UIs use `$peekaboo`. CLIs and TUIs use a terminal or PTY. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
- Broken skill mid-task → fix it locally without blocking or silently working around it. Open a separate PR only when the user authorized PR creation.
- Long, autonomous, or multi-phase work, or any task the user steps away from to review later ("going to bed", "trust it when i'm back", "/goal until X") → a decision trail via the **show-me-your-work** skill. Commit it when stakes need an auditable record; keep it local otherwise.
...
**Just do it.** Use any available tool. Reversible local work inside the user's requested scope proceeds without asking.

**Always pause** for irreversible writes and external actions the user did not request: force-push to shared branches, deploys, data deletion, customer messages, team chat, ticket updates, PR creation, pushes, and merges.
...
- **Autonomous run.** A long task to drive to completion without stopping ("run until done", "/goal until X"). `playbooks/autonomous-run.md`.
