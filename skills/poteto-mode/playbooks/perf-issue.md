...
1. Capture a baseline trace via the matching verification skill or tool.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using `gpt-6-astra` at medium reasoning. Use high when the implementation still requires complex performance or concurrency reasoning. Review the diff. Capture a post-fix trace.
