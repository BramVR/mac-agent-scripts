...
3. Make the work durable. When commits are authorized, commit uncommitted edits as one clear `wip:` commit on the current branch so nothing is lost. If the tree is broken, say so in the commit body in one line. Otherwise leave the edits in place and record their exact paths and state in the resume note.
