...
Open a plan with the steps below copied in verbatim. A step you skip stays listed with `skip: <reason>`.

#### Roles and placement

- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. When commits and pushes are authorized, mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the Codex collaboration tools. State reads and writes go through `${CODEX_HOME:-$HOME/.codex}/skills/poteto-mode/scripts/dist/orch` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works within Codex's available collaboration depth and concurrency). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Use Codex collaboration subagents in standard mode. Workers use `gpt-6-astra` at medium reasoning, or high for reasoning-heavy work under **Model routing**. Procedural or live verifiers use `gpt-5.6-luna` at high reasoning; judgment-heavy or high-blast-radius final verifiers use `gpt-6-astra` at high reasoning. Never use fast mode. Runtime verification uses the matching project `verify-<app>` skill, `$browser-use`, `$peekaboo`, or a terminal or PTY; local transcripts are exact current-thread rollouts under `${CODEX_HOME:-$HOME/.codex}/sessions/`; simulators and local IDE state stay local. Subagents share the checkout and can read the local store. Prefer fewer, broader workers; one writer per disjoint file set. Serialize shared writes unless the user explicitly authorized isolated worktree tasks (principle-separate-before-serializing-shared-state). Run a unit's verifier in an independent fresh context from its worker.
...
Create `${CODEX_HOME:-$HOME/.codex}/poteto-mode/orchestrate/<project-slug>/` and pass it to every `orch` command as `--store <path>` or `ORCH_STORE`. Every file has exactly one writer; owners publish facts, readers aggregate at read time. Use `${CODEX_HOME:-$HOME/.codex}/skills/poteto-mode/scripts/dist/orch` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
SCOPE        paths this unit may write; paths it may not; its exclusive file set or branch
...
VERIFY       exact commands or the verification skill or tool path, plus known gotchas
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Spawns may reference the standing-orders file by store path when they share the local filesystem; paste it verbatim for every other spawn and every resume.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget with `gpt-6-astra` medium reasoning for workers, with high reserved for reasoning-heavy work under **Model routing**, `gpt-5.6-luna` high reasoning for procedural or live verifiers, and `gpt-6-astra` high reasoning for judgment-heavy or high-blast-radius final verifiers, plus the local-surface exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
This orchestration runtime is GitHub-only. If the active forge is Origin, stop before initialization and report that the runtime is unsupported.
...
2. **Install the runtime.** Run `orch --store <path> init`. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from the explicit bottom-to-top PR order with `orch --store <path> frontier set --repo <repo-dir> --prs <n,...>`.
...
7. **Close.** Drain the final inbox, reconcile every spawned agent to a terminal row (done, abandoned, zombie-reconciled), confirm the predicate on the real artifact, confirm every landed PR has a verdict for its current head SHA, audit the trail per show-me-your-work including its independent-context review, encode recurring corrections into `preferences.md` or the brief template. Leave the store intact; it is the postmortem.
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it through Codex Goal mode with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` from `gh` and local git after every merge and stack mutation because GitHub base refs drift mid-restack: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Resolve it from the active forge's PR bases plus fetched local refs; if either source is incomplete, the command errors rather than guessing.
- Exactly one stacker per stack may mutate branch topology, serialized within its stack; record the holder in the standing orders. Use an isolated worktree task for restacks only when the user explicitly authorized it.
- Workers never rebase and never mutate stack topology. Babysitters follow `playbooks/babysit.md`, one per stack, scoped to one immutable frontier generation; they report conflicts to the stacker rather than restacking.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts; a dedicated verifier agent in an independent fresh context uses `gpt-5.6-luna` for procedural or live verification and `gpt-6-astra` at high reasoning when verification is judgment-laden or high-blast-radius. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.
...
A unit is not done until its output is externalized the moment it lands, never batched to the end of the run: a worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in one subagent context when that context dies was never done.

#### Liveness and failure

- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the subagent status exposed by Codex. Transcript mtime is not liveness.
...
- Retry by mode: cap-hit or oom, respawn with smaller scope; network-drop, retry as-is; tool-error, retry in a fresh context on the same assigned model; unknown, retry once. Two retries, then abandon the unit and replan around it.
...
- After a Codex restart: treat local collaboration agents as dead and re-read external PR and branch state. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.

#### Escalation

Reaches the human, batched into the status page rather than per item: irreversible actions and external writes the user did not request (force-push to shared branches, deploys, deletions, closing someone else's PR, pushes, PR creation, comments, and merges), genuine product or preference calls no experiment settles, a standing order that contradicts observed reality, a program-level dead end that survived a replan. Park each as a `gates.md` entry before asking, and route work around it.
