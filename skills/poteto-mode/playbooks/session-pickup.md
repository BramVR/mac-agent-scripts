...
1. Locate the prior trail. A local Codex rollout under `${CODEX_HOME:-$HOME/.codex}/sessions` (do not read other projects' rollouts; that crosses project boundaries and reads private chats from unrelated work), a Codex cloud task URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
