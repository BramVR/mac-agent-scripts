...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a Codex task or share URL handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. An exact current-thread rollout under `${CODEX_HOME:-$HOME/.codex}/sessions`, resolved from `CODEX_THREAD_ID` without reading unrelated sessions; a Codex task or share URL; or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
