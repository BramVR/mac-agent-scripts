...
1. Read `references/comment-sicko.md` and pass it verbatim with the scope to a Codex collaboration agent using `model: "gpt-5.6-luna"`, `reasoning_effort: "high"`, and `fork_turns: "none"`. Never use fast mode or substitute another model or reasoning level. Do not restate its rules. After its required first response, send the same agent one follow-up telling it to perform the requested review on the scope already supplied.
