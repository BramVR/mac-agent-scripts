...
Open a plan whose first item is to read the Principles section of the **poteto-mode** skill. Then add the phases below as plan items.
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with isolated, opinionated candidates and a separately prompted judge, all using `gpt-6-astra` at `high` reasoning and never fast mode. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
