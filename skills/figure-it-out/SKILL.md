...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with isolated, opinionated candidates and a separately prompted judge, all using `gpt-5.6-sol` at `high` reasoning and never fast mode. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
