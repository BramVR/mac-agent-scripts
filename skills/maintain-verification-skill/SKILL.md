...
description: "Audit a project-local verification skill and feature map against current source and live behavior."
---
...
- **changed** — proven doc, harness, or map corrections are ready. Open one PR only when the user asked for it.
- **blocked** — coverage could not finish or a proven fix could not be made safely. Say exactly what blocked it.
...
2. **Source wave.** One agent per feature file, launched in concurrent waves of up to 3. Each uses `model: gpt-5.6-luna`, `reasoning_effort: high`, and `fork_turns: none`. Never use fast mode or substitute another model or reasoning level. Codex has no read-only switch for collaboration agents, so explicitly instruct each agent not to edit files, change repository state, or perform external writes. Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe. Children never drive the app and never edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe.
...
6. **Ship or stop.** For changed: re-read every changed file first and prepare the proven corrections. Open one PR only when the user requested it. For clean or blocked: no PR, report the outcome and the coverage honestly.
