---
name: how
description: "Use for code walkthroughs, subsystem architecture, runtime flow, ownership, layering, or architecture critique."
---
...
Decompose the question into 2-3 parallel exploration angles, each a distinct slice of the subsystem so explorers don't duplicate work. Example split for "how does the rate limiter work?":

- Explorer 1: data model and state management
- Explorer 2: request path and enforcement
- Explorer 3: configuration and metrics infrastructure

The right decomposition depends on the question. Use your judgment. Narrow questions: 2 explorers is fine. Broad subsystems: up to 3.

Spawn all explorers concurrently. Each uses the explorer contract above.

Each explorer gets the same base prompt from `references/explorer-prompt.md` plus a specific exploration angle naming its slice. Each explorer should:
- Start broad: use `rg --files` for relevant directories and files, `rg` for key types/interfaces/class names
...
Spawn a single collaboration agent that explores and explains in one pass. Use the explainer contract above.

The agent does its own exploration (`rg --files`, `rg`, targeted reads) and writes the explanation directly. Read `references/explainer-prompt.md` for the communication style and output format. Same structure, just no explorer findings as input.

Proceed to Step 4.

### Step 3. Synthesize (complex questions only)

Once all explorers return, spawn a single collaboration agent to synthesize their findings into one coherent explanation. Use the synthesizer contract above.
