...
| Reviewer C | `claude -p --model fable --effort high` |

For each reviewer:
- `model`: the model assigned in the table
- `reasoning_effort`: the level assigned in the table
- `fork_turns`: `none`
- Explicitly instruct the reviewer not to edit files, change repository state, or perform external writes.
